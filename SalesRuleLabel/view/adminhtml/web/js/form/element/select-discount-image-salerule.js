    ...
    return select.extend({
        ...
        initialize: function() {
            ...
            if (imageId) {
                ...
                if (imageId > 0) {
                    var custom_controller_url = url.build('getimage?id=' + imageId);
                    custom_controller_url = custom_controller_url.replace('mit_salesrulelabel/index/index', 'mit_salesrulelabel/index/getimage');
                        ...
                        .done(function(response) {
                            if (response && response.res) {
                                setTimeout(function() {
                                    ...
                                    let label_color = $('input[name="discount_label_color"').val();
                                    let label_style = $('textarea[name="discount_label_style"').val();
                                    ...
                                    let body = ' <div class="ds-main"><div class="custom-ds-lbl ' + response.res.main_class + '"> \
                                                ...
                                                <span class="discount-txt ' + response.res.sub_class + '" ' + labelStyles + ' > ' + $('input[name="discount_label"').val() + ' </span> \
                                            </div></div> \
                                    ';
                ...
                if (imageId > 0) {
                    ...
                    $.post(custom_controller_url, 'json')
                        .done(function(response) {
                            ...
                            ';
                                $('select[name="discount_image_id"').parent().append(body);
                                $('input[name="width"').val(response.res.width).change();
                                $('input[name="height"').val(response.res.height).change();
                            }
                        });
                } else {
                    $('.ds-main').remove();
